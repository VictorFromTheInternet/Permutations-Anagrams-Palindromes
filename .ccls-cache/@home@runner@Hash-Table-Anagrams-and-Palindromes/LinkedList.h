...
// defintion of the linked list iterator 
template <class Type>
...
public:
    ...
    //////////////////////////////////////
    // added stuff for reverse printing //
    //////////////////////////////////////
...
{
    nodeType<Type> *current; //pointer to traverse the list, keeps track of where we are

    current = first;    // set current so that it points to 
                        // the first node
    while (current != nullptr) // continue until current doesnt == nullpointer/last item
    {
        cout << current->info << " ";
        current = current->link;
    }
...
{   
    assert(first != nullptr); // checking to see if the list is empty, ends the program if it is

    return first->info; //return the info of the first node	if full
}//end front
...
{
	// check to see if weve not reached the end of the list (base case)
	if(current != nullptr)
	{
		reversePrint(current -> link); // recursive call
		
		// ^once it gets to the base case (last node in the list)
		// outputs the info of the node
		cout << current -> info << " "; 
	}
}
